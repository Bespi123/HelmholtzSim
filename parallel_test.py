...
def process_data_in_chunks(data, chunk_size=1000):
    ...
    with tqdm(total=total_chunks, desc="Processing Chunks") as pbar, ProcessPoolExecutor(max_workers=cpu_count()) as executor:
        for i in range(0, len(data), chunk_size):
            chunk = data[i:i + chunk_size]
            chunk_results = list(executor.map(biot_Savart_law, chunk))
            results.extend(chunk_results)
...
def generate_arguments(grid_number, A, coil_params, current):
    ...
    A1 = (coil_params.N * MU_0 * current) / (4 * np.pi)
    A1_arr = np.full((all_points_repeated.shape[0], 1), A1)

    data = np.hstack((A1_arr, all_points_repeated, all_coils_repeated, diff_coils_repeated))
    return data
