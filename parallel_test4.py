...
def coil_simulation_1d_sequential(X, Y, Z, coil_params, current, coil1, coil2, parallel_coils, batch_size):
    """
    ...
    num_iter = len(X_flat) + len(Y_flat)
    ...
    for k in range(0, len(X_flat), batch_size):
        ...
        P_batch = np.stack([
            np.stack([X_batch, Y_batch, Z_batch], axis=1),  # P1: X-Y plane
            #np.stack([_batch, X_batch, Y_batch], axis=1),  # P2: Y-Z plane
            #np.stack([X_batch, Z_batch, Y_batch], axis=1)   # P3: X-Z plane
