...
class CoilParameters:
    ...
    def square_spires(self, num_seg, b=None):
        ...
        first_points = spires[:, :, 0:1]
        closed_spires = np.concatenate([spires, first_points], axis=2)

        return closed_spires
    ...
    def circular_spires(self, num_seg):
        ...
        for i in range(self.coils_number):
            ...
            # close the spire by repeating the first point
            closed_spire = np.concatenate([spire, spire[:, 0:1]], axis=1)

            displacement = np.array([self.pos[i], 0, 0]).reshape(3, 1)

            # Aplicar transformación con matriz A y desplazar
            spire = np.einsum('ij,jk->ik', self.A, closed_spire - displacement)
...
def calculate_field(args):
    """
    Calculates the total magnetic field generated by a set of coils at a single point in space.

    This function implements a vectorized version of the Biot-Savart law for each coil
    and sums the contributions from all coils (principle of superposition).

    Parameters
    ----------
    args : tuple
        A tuple containing all necessary parameters for the calculation:
        P (numpy.ndarray): The observation point (x, y, z) where the field is calculated. 
                           Shape: (3,).
        all_coils (numpy.ndarray): A 3D array with the geometries of all coils.
                                   Shape: (num_coils, 3, num_points).
        all_turns (numpy.ndarray): A 1D array with the number of turns for each coil.
                                   Shape: (num_coils,).
        I (float): The current flowing through the coils (assumed to be the same for all).

    Returns
    -------
    numpy.ndarray
        The total magnetic field vector (Bx, By, Bz) at point P. Shape: (3,).
    """
    P, all_coils, all_turns, I = args
    total_B = np.zeros(3)
    
    # Loop through each coil and sum its contribution to the total field
    for i in range(len(all_coils)):
        single_coil = all_coils[i]
        N = all_turns[i]
        
        # Biot-Savart constant for the current coil
        A1 = (N * MU_0 * I) / (4 * np.pi)
        
        # Calculate the differential length segments (dl) of the coil
        dl = np.diff(single_coil, axis=1)
        
        # Calculate the position vectors (R) from each segment to point P
        R = P[:, np.newaxis] - single_coil[:, :-1]
        
        # Calculate the magnitude of R and avoid division by zero
        norm_R = np.linalg.norm(R, axis=0)
        norm_R = np.where(norm_R < 1e-9, 1e-9, norm_R)
        
        # Apply the Biot-Savart law in a vectorized way
        dB = (A1 * np.cross(dl, R, axis=0)) / (norm_R**3)
        
        # Sum the contribution of all segments of this coil
        total_B += np.sum(dB, axis=1)
        
    return total_B


def magnetic_field_coil_parallel(P, N, I, spires_np):
    """
    Calculates the total magnetic field at multiple observation points in parallel.

    This function acts as an orchestrator for the simulation. It prepares the data
    for each observation point and distributes the calculation across all available
    CPU cores using the `multiprocessing` module. Each parallel task calls the
    `calculate_field` function to get the total magnetic field (the sum of contributions
    from all coils) at a single point in space.

    Parameters
    ----------
    P : numpy.ndarray
        A matrix with the coordinates of the observation points (x, y, z).
        Shape: (m, 3), where 'm' is the number of points.
    N : numpy.ndarray
        A 1D array with the number of turns for each coil.
        Shape: (num_coils,).
    I : float
        The current, in Amperes, flowing through the coils.
    spires_np : numpy.ndarray
        A 3D array with the geometries of all coils.
        Shape: (num_coils, 3, num_points_per_coil).

    Returns
    -------
    numpy.ndarray
        A matrix with the total magnetic field vector (Bx, By, Bz) calculated
        at each observation point P. Shape: (m, 3).
    """

    # Prepare arguments for each parallel task.
    args = [(P[i, :], spires_np, N, I) for i in range(P.shape[0])]
    
    # Create the process pool (one for each CPU core) only once.
    with Pool(processes=cpu_count()) as pool:
        # Map the calculation function to all points
        B_results = pool.map(calculate_field, args)
    # Convert the list of results into a NumPy array and return it.
...
def coil_simulation_parallel(X, Y, Z, coil_params, spires_np, batch_size, enable_progress_bar=True, n=100):
    ...
    # map the spires differential with number of turns
    ...
    for k in range(0, len(X_flat), batch_size):
        ...
        B = magnetic_field_coil_parallel(P_batch, coil_params.N, coil_params.I, spires_np)
