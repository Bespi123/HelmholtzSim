...
2. Install dependencies:
   ...
   for conda users:
    ...
    ```
    The created environment is called helm_simulator by default

## Mathematical basis: The Biot-Savart law 
The simulation is based on the **Biot–Savart law**, which calculates the magnetic field **B** generated by a current **I** flowing through a wire segment. 

### Continuous Form:

$$
\mathbf{B} = \frac{N \mu_0}{4\pi} \int \frac{I \mathbf{dL} \times \hat{\mathbf{R}}}{\|\mathbf{R}\|^2}
$$

Where:

- $\hat{\mathbf{R}}$: Unit vector in the direction of $\mathbf{R}$
- $\|\mathbf{R}\|$: Magnitude (norm) of vector $\mathbf{R}$
- $N$: Number of turns in the coil
- $\mu_0$: Magnetic permeability of vacuum
- $I$: Current in the coil (in amperes)
- $\mathbf{dL}$: Differential segment of the wire
- $\mathbf{R}$: Vector from $\mathbf{dL}$ to the evaluation point

### Discrete Approximation:

To make the solution suitable for general-purpose simulations, the continuous integral is approximated as a discrete sum:

$$
\mathbf{B}(\mathbf{r}) = \frac{N \mu_0}{4\pi} \sum \frac{I \Delta \mathbf{L} \times (\mathbf{r} - \mathbf{r'})}{\|\mathbf{r} - \mathbf{r'}\|^3}
$$

- $\mathbf{r}$: Evaluation point
- $\mathbf{r'}$: Location of current element
- $\Delta \mathbf{L}$: Finite segment aligned with current

For better understanding, we propose the following diagram.


![Discrete Biot–Savart illustration](figs/fig1.jpg)
